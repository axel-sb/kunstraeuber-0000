...
import  {
 ...
 Form, Link, useLoaderData } from '@remix-run/react'
import { Button } from '#app/components/ui/button.tsx'
import { Icon } from '#app/components/ui/icon.tsx'
import { Input } from '#app/components/ui/input.tsx'
import { LocalStorageDB } from '#app/components/ui/LocalStorageDB.tsx'
import { ComboboxMultiple } from '../../components/ui/combobox-multiple.tsx'
import { getMeta } from '../../components/ui/getMeta.tsx'
import { InlineCode } from '../../components/ui/InlineCode.tsx'
import {
	Tabs,
	TabsList,
	TabsTrigger,
...
import testStyles  from './test.css?url'

export const links: LinksFunction = () => {
	return [
		// Preload svg sprite as a resource to avoid render blocking
		{ rel: 'preload', href: testStyles },
    ]
}
		...
		files: [
			{
				...
				content: await readFile(
					path.resolve(__dirname, '../../components/ui/combobox-multiple.tsx.sly'),
					'utf8',
				),
			},
		],
			...
			<div className="mt-6">
				<Tabs defaultValue="preview">
					...
					<TabsContent value="preview">
						<Form
							method="POST"
							className="shadow-smooth flex justify-center gap-x-2 rounded-md p-8"
						>
							<>
								{genres ? (
									<ComboboxMultiple
...
/>`}</code>
			...
			<div className="mt-6">
				<Tabs defaultValue="preview">
					...
					<TabsContent value="preview">
						<div className="shadow-smooth rounded-md">
							<Form method="POST" className="flex justify-center gap-x-2 p-8">
								<>
									{genres ? (
										<ComboboxMultiple
											className="max-w-xs grow border border-slate-300"
											name="genreId"
											defaultValue={selectedGenreIds}
											options={genres?.map((genre) => ({
												label: genre.name,
