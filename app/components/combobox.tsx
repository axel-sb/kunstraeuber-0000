...
export function SearchCombobox({ error }: { error?: string | null }) {
	...
	return (
		<div className="relative">
				...
				<input
					{...cb.getInputProps({
						className: clsx('w-full border border-gray-500 px-2 py-1 text-lg', {
							'rounded-b-0 rounded-t': displayMenu,
							rounded: !displayMenu,
						}),
			...
			<ul
				{...cb.getMenuProps({
					className: clsx(
						'absolute z-10 max-h-[180px] w-full overflow-scroll rounded-b border border-t-0 border-gray-500 bg-white shadow-lg',
						{ hidden: !displayMenu },
					),
				})}
			>
